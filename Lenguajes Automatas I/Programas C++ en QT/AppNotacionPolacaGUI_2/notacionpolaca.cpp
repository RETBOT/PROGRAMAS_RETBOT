...
QString NotacionPolaca::ConvertirAPosFijaS(QString EI)
{
     string EI2 = EI.toStdString();
    char *Epos = new char[EI2.length()];
    int cantidad = contOperadores(EI2);

    PilaT<int> *stack = new PilaT<int>(cantidad);

    int i,pos = 0;
    for(i=0;i < (int)EI2.length();i++){
        char car = EI2[i];
        Simbolo actual = TipoYPrecedencia(car);
        switch (actual) {
        case OPERANDO: Epos[pos++] =car; break;
