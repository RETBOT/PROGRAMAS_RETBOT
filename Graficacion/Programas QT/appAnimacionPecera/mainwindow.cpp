    ...
    , ui(new Ui::MainWindow)
{
    ui->setupUi(this);
    timer = new QTimer();
    connect(timer,SIGNAL(timeout()),this,SLOT(moverImgaen1()));
}
...
void MainWindow::moverImgaen1()
{
    ...
    if(ui->label->geometry().x() <
           (ui->pecerawidget->geometry().width() - ui->label->geometry().width()) && band){
        ui->label->setGeometry(x,y,ui->label->width(),ui->label->height());
        x+=10;
        if(ui->label->geometry().x() >=
                ui->pecerawidget->geometry().width() - ui->label->geometry().width()){
